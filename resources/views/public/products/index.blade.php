...
@section('content')
    {{-- Page Header --}}
    <section class="relative bg-slate-900 py-20 lg:py-28 overflow-hidden section-anchor" id="catalog-intro">
        <div class="absolute inset-0">
            <img src="{{ asset('images/approach.jpeg') }}" alt="Catalog Background" class="w-full h-full object-cover opacity-30">
            <div class="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/50 to-transparent"></div>
        </div>
        <div class="container-custom relative z-10 text-center">
            <h1 class="text-4xl md:text-5xl md:leading-tight font-serif font-bold text-white mb-4 scroll-fade-up">Katalog Produk</h1>
            <p class="text-lg text-slate-300 max-w-2xl mx-auto font-light scroll-fade-up delay-100">
                Temukan perlengkapan event berkualitas premium untuk menyempurnakan acara spesial Anda.
            </p>
        </div>
    </section>
    ...
    <section class="py-16 bg-white">
        <div class="container-custom">
            <div class="flex flex-col lg:flex-row gap-12">
                {{-- Sidebar Categories --}}
                <aside class="lg:w-64 flex-shrink-0 scroll-fade-up">
                    <div class="sticky top-24">
                        <h3 class="font-serif text-xl font-bold text-slate-900 mb-6 pb-4 border-b border-slate-200">Kategori</h3>
                        <ul class="space-y-3">
                            <li>
                                <a href="{{ route('products.index') }}"
                                    class="group flex items-center justify-between text-sm transition-colors {{ !request()->route('slug') ? 'text-primary-600 font-medium' : 'text-slate-600 hover:text-primary-600' }}">
                            ...
                            @foreach($categories as $cat)
                                <li>
                                    <a href="{{ route('products.category', $cat->slug) }}"
                                        class="group flex items-center justify-between text-sm transition-colors {{ request()->route('slug') == $cat->slug ? 'text-primary-600 font-medium' : 'text-slate-600 hover:text-primary-600' }}">
                                        <span>{{ $cat->name }}</span>
                                        <span class="text-xs text-slate-400 group-hover:text-primary-400 bg-slate-100 group-hover:bg-primary-50 px-2 py-0.5 rounded-full transition-colors">{{ $cat->active_products_count ?? 0 }}</span>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                        ...
                        @empty
                            <div class="col-span-full flex flex-col items-center justify-center py-20 text-slate-400">
                                ...
                                <p class="text-lg font-light">Belum ada produk tersedia dalam kategori ini.</p>
